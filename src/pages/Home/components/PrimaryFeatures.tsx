...
function InviteScreen({ custom, animated = false }: ScreenProps) {
  return (
...
function StocksScreen({ custom, animated = false }: ScreenProps) {
  return (
    <AppScreen className="w-full">
      <MotionAppScreenHeader {...(animated ? headerAnimation : {})}>
...
function InvestScreen({ custom, animated = false }: ScreenProps) {
  return (
    <AppScreen className="w-full">
      <MotionAppScreenHeader {...(animated ? headerAnimation : {})}>
...
function FeaturesMobile() {
  ...
  React.useEffect(() => {
    const _refs = [] as React.RefObject<HTMLDivElement>[];
    for (let k = 0; k < features.length; k++) {
      _refs.push(slideRefs[k] || React.createRef())
    }
    // add or remove refs
    setSlideRefs(_refs);
  }, [features]);
      ...
      (entries) => {
        for (let entry of entries) {
          if (entry.isIntersecting) {
            setActiveIndex(slideRefs.findIndex(i=>i.current===entry.target));
            break
          }
        }
      },
      ...
      <div className="mt-6 flex justify-center gap-3">
        {features.map((_, featureIndex) => (
          <button
            ...
            onClick={() => {
              const i = slideRefs[featureIndex];
              (i && i.current) && i.current.scrollIntoView({
                block: 'nearest',
                inline: 'nearest',
              })
            }}
